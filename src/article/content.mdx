...
import HMMTagGraph from './partII/tagGraph'
...
import MEMMLabelBiasGraphFull from './partIII/labelBiasGraphFull'
import MEMMLabelBiasGraphS from './partIII/labelBiasGraphS'
import MEMMLabelBiasGraphP from './partIII/labelBiasGraphP'
import MEMMLabelBiasGraphL from './partIII/labelBiasGraphL'
...
<Banner>
	<strong>Tip:</strong> refer to the glossary in the top right corner for definitions of
	key terms like “named entity”.
</Banner>
}
...
<LivePrediction
	...
	label={<Fragment>Which of these words refer to named entity?</Fragment>}
/>
...
Throughout this article, we’ll work with the CoNLL-2003 English dataset [@Sang2003]. The dataset recognizes 9 possible name tags. Each, apart from the “O” tag, has either a “B-” (for beginning) or “I-” (for internal/inside) prefix, to eliminate confusion about when an entity stops and the next one begins. For example:
...
<LivePrediction
	...
	initialInputValue="The London-to-Boston route"
/>
...
MEMMs’ discriminative structure offers great benefits as demonstrated above, but it comes with a downside: the label bias problem. First reported by Bottou [@Bottou1991], label bias mostly affects discriminative models, causing certain states to effectively ignore their observations, leading to demonstrably higher error rates [@Lafferty2001].
...
$$
\sum_s{p_{s\prime}(s|o)} = 1
$$

This requirement is satisfied by _local_ normalization via the term $Z(o,s\prime)$ baked directly into the definition of $p_{s\prime}(s|o)$:
...
Let’s assume that there are only two hidden states in our model: “L” if the word is part of a location name, and “P” if it’s part of a person’s name. There’s a special state “S” for the start of the sentence. We observe the two-word sequence “Stone Creek”. The transition graph is the following:

<MEMMLabelBiasGraphFull />

At inference time we proceed from left to right. The first observation is “Stone”. We need to find $p_{\text{S}}(\text{L}|\text{“Stone”})$ and $p_{\text{S}}(\text{P}|\text{“Stone”})$. Let’s say that they both have the same value, $0.5$:

<MEMMLabelBiasGraphS />

We then go to the second observation, “Creek”. The first pair of transition probabilities that we need to calculate is $p_{\text{L}}(\text{L}|\text{“Creek”})$ and $p_{\text{L}}(\text{P}|\text{“Creek”})$. Under local normalization, the two must sum up to one. Let’s say that they come out to $0.9$ and $0.1$ respectively.

<MEMMLabelBiasGraphL />

So far so good. Now we need to calculate $p_{\text{P}}(\text{L}|\text{“Creek”})$ and $p_{\text{P}}(\text{P}|\text{“Creek”})$. Let’s say probabilities are, in turn, $0.05$ and $0.95$.

<MEMMLabelBiasGraphP />

Now, intuitively the word “Creek” is much more likely to be part of a location name than a person’s name. But here the first set of transition probabilities, $p_{\text{L}}$ — $\{0.9, 0.1\}$, have the same total mass as the second set, $p_{\text{P}}$ — $\{0.05, 0.95\}$. Both sum up to one because of local normalization. The result is that taken together, the model judges paths that go through the “L” state first (i.e. “S” → “L” → “L” and “S” → “L” → “P”) to be about as likely as those that go through the “P” state first (“S” → “P” → “L” and “S” → “P” → “P”), contradicting our intuition about “Creek” being more consistent with the former than the latter. The model has in effect ignored crucial information contained within the observation “Creek”.

More generally, due to MEMMs’ discriminative structure, observations can only affect how the incoming probabilities are distributed across the possible current states, but not how much total probability mass to pass on. That total mass must always equal one, due to local normalization. In the example above, given the previous state “P”, the observation “Creek” may shift the distribution over the current state toward $\{0.05, 0.95\}$ rather than some other split like $\{0.1, 0.9\}$, but it cannot change the total probability mass that the previous state “P” passes on. We can’t have the transition probability pair be, say, $\{0.01, 0.19\}$, which would better reflect our intuition about “P” being less likely under “Creek”.

So that is the crux of the label bias problem. Local probability normalization causes MEMMs to discard useful information about how likely state transitions are under a given observation. The problem extends beyond the simple case above. Hannun [@Hannun2019] offers an excellent, more detailed explanation of the problem and how it generalizes to more scenarios and model structures.
...
CRFs are a type of Markov Random Fields (MRFs) — probability distributions over random variables described by _undirected_ graphs [@Blake2011], for example:
...
The factors $\phi$ promote assignments in which their constituent variables ($A$ and $B$ in the case of $\phi(A, B)$) agree with each other. The assignment 1-1-1-0 (for $A$, $B$, $C$, and $D$ respectively) would receive a higher score and thus have higher probability than say 1-1-0-0, since there is more agreement in the former case.
...
Global conditioning on $x$ helps CRFs avoid the label bias problem [@McCallum2000]. As discussed above, the problem mostly affects models with local probability normalization, including MEMMs [@Hannun2019]. CRFs avoid this since they model the probability of the entire state sequence $p(y|x)$, which has a single, global normalization term $Z$:
...
<LivePrediction
	models={['crf']}
	initialInputValue="Czech President Vaclav Havel"
	label="Name tag predictions by unary CRF:"
/>
...
<LivePrediction
	hideTagPrefixes
	models={['hmm', 'memm', 'crf']}
	initialInputValue="Suu Kyi's National League for Democracy"
	label={<Fragment>Which of these words refer to named entity?</Fragment>}
/>

The work continues. CRFs remain a powerful tool for not only language modeling but also various image recognition tasks [@Sutton2010]. In 2016, Zheng et al. [@Zheng2015] reinterpreted CRFs as recurrent neural networks (CRF-RNNs) and demonstrated their effectiveness in semantic image segmentation — an encouraging result and compelling example of the theoretical breadth of CRFs specifically and of graphical models more generally.
