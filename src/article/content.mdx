...
\footnotesize\textrm{for all $i\,{\scriptstyle \in}\,\{2,…, N\}$}
$$
...
\footnotesize\textrm{for all $i\,{\scriptstyle \in}\,\{2,…, N-1\}$}
$$
...
\footnotesize\textrm{for all $i\,{\scriptstyle \in}\,\{1, 2,…, N\}$}
$$
...
At inference time we proceed from left to right. The first observation is “Stone”. We need to find $p_{\text{S}}(\text{LOC}|\text{“Stone”})$ and $p_{\text{S}}(\text{PER}|\text{“Stone”})$. Let’s say that they both have the same value, $0.5$:
...
$$
p(x_1, x_2,…) = \frac{1}{Z}\prod_{c\,{\scriptstyle \in}\,C}{\phi_c(x_c)} \\
...
$$
p(y|x) = \frac{1}{Z(x)}\prod_{c\,{\scriptstyle \in}\,C}{\phi_c(y_c, x_c)} \\
\footnotesize\textrm{where Z is a normalization term} \\
